...
public class MyStateTransition implements StateTransition<BobState> {

    private final FoodSubsystem foodSubsystem;

    public MyStateTransition(FoodSubsystem foodSubsystem) {
    ...
    @Override
    public BobState parse(BobState current) {
        return switch (current) { //interior class overview
            case HUNGRY -> hungry();
            case EATING -> eating();
            case FULL -> full();
        };
